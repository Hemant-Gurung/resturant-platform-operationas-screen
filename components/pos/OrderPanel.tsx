  ...
  return (
    <div className="w-56 bg-white border-l-2 border-[#f0e8e0] flex flex-col p-4 shrink-0">
      <div className="shrink-0 mb-3">
        <p className="font-mono font-bold text-[#2d2d2d] text-sm tracking-wide">ORDER</p>
        <p className="text-xs text-[#aaa] mt-0.5">New order</p>
      </div>

      <div className="flex-1 overflow-y-auto min-h-0" style={{ scrollbarWidth: 'none' }}>
        {items.length === 0 ? (
          <p className="text-xs text-[#ccc] text-center pt-6">Add items from the menu</p>
        ) : (
          items.map((item) => (
